...
function TagList({ items }: { items: string[] }) {
  return (
    <View className="mt-2 flex-row flex-wrap gap-2">
      {items.map((item) => (
        <View key={item} className="rounded-full border border-app-border bg-app-card px-3 py-1.5">
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <View className="mb-3">
      <Text className="text-xs uppercase tracking-[1.4px] text-app-muted">{label}</Text>
  ...
  return (
    <View>
      <Card className="mb-4 rounded-3xl border-app-border bg-app-card px-4 py-4">
        <Text className="text-base font-bold text-app-foreground">{PROFILE_DETAILS_COPY.ABOUT_TITLE}</Text>
